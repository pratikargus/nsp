...
var onCommand = function (args) {
  ...
  if (args.pkgPath) {
    pathToPackage = args.pkgPath;
  } else {
    pathToPackage = process.cwd();
  }

  var pkgPath = Path.join(pathToPackage, 'package.json');
  var shrinkwrapPath = Path.join(pathToPackage, 'npm-shrinkwrap.json');
  var packageLockPath = Path.join(pathToPackage, 'package-lock.json');
